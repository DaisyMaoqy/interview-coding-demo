...
- **统计报表** —— 主管专属管理视图，展示团队差旅成本、人均花费与审批效率（员工与财务不可见）
...
│   └── /:id                申请详情
└── /travel/approvals       待我审批（仅主管与财务可见）

/apply/[type]/[step]        统一发起向导
                            [type] = travel | leave，[step] = basic | trips | budget | preview
                            左侧导航顶部可切换申请类型，向导的步骤与字段完全由注册表驱动

/reports                    统计报表
```

「我的申请」「待我审批」收敛在 `/travel` 下，统计独立为 `/reports`，
